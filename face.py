...
import threading
import queue
from constants import TOKEN, NASIBA
from unknown_faces_handler import stats_menu, confirm_add_student, settings_menu, tolerance_menu
from unknown_faces_handler import unknown_faces_saver, unknown_faces_sender
...
# Define a function to update the known face encodings list
def update_known_face_encodings():
    global unknown_face_encodings
    while True:
        try:
            student_data = student_queue.get(timeout=1)  # Get data from the queue
...
def reg_name(message):
    ...
    bot.send_message(admin, f'Name of the newly added student is {message.text}, correct?', reply_markup=confirm_add_student)
...
def start(message):
    ...
    bot.reply_to(message, f"Hello! I am a bot, which will provide with statistics about students'" 
                          "attendance")
